...
Mount tables can be iterated with B<getmntent> and paths can be
translated to device files (or whatever the actual B<quotactl>
implementations needs as argument) of the respective file system.
...
The function returns an iterator that can be used to list all entries
in the mount table. Each returned object is a tuple with the following
entries of type string:
...
Examples for all functions can be found in the test scripts.
