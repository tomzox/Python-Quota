...
=head1 SYNOPSIS
    ...
    (block_curr, bsoft, bhard, btimelimit,
     inode_curr, isoft, ihard, itimelimit) =
...
Instances of the I<MntTab> class allow iterating across the mount
...
=item I<(bcurr,bsoft,bhard,btime, icurr,isoft,ihard,itime)> =
I<qObj.query(uid, [keyword_options...])>
...
In I<bcurr> and I<icurr> the current usage in blocks and inodes is
returned.  I<bsoft> and I<isoft> are the soft limits, I<bhard> and
I<ihard> hard limits. If the soft limit is exceeded, writes by this user
will fail for blocks or inodes after I<btime> or I<itime> is reached.
These times are expressed as usual, i.e. in elapsed seconds since 00:00
1/Jan/1970 GMT.

Note: When the quota limits are not exceeded, the timestamps are
meaningless and should be ignored. When hard and soft limits are both
zero, this means there is no limit for that user. (On some platforms the
query may fail with error code I<ESRCH> in that case; most however still
report valid usage values in that case.)
...
When parameter B<grpquota> is present and set to True, the value in I<uid>
...
When parameter B<prjquota> is present and set to True, project quotas are
queried; this is currently only supported for XFS. Exception
B<FsQuota.error(ENOTSUP)> is raised for unsupported file-systems.

=back

=item I<qObj.setqlim(uid, bsoft, bhard, isoft, ihard [,keyword options...])>

Sets quota limits for the given user. Meanings of parameters I<uid>,
I<bsoft>, I<bhard>, I<isoft> and I<ihard> are the same as for the B<query>
method.
...
I<qObj.setqlim(uid, isoft=10,ihard=20)> would allow unlimited blocks and
limit inode counts to 10 soft, 20 hard.
...
=item B<timelimit_reset={0|1}>

Optional parameter I<timelimit_reset> defines how time limits are
initialized: I<0>: The time limits are set to B<NOT STARTED>, i.e. the
time limits are not initialized until the first write attempt by this
user.  This is the default when the parameter is omitted.  I<1>: The time
limits are set to B<7.0 days>.  More alternatives (i.e. setting a specific
time) aren't available in most implementations.

=item B<grpquota=1>

When parameter B<grpquota> is present and set to True, parameter I<uid> is
...
Sets the port used by I<rpc.rquotad(8)>; default value is zero, which
which means the remote host's portmapper (aka rpcbind) is used. (Note
in case of the latter you can find out the port using I<rpcinfo -p host>)
...
=item I<iterator = FsQuota.MntTab()>
...
Name of the filesystem (e.g. device name)

=item [1] B<mnt_dir>

Filesystem path prefix (aka mount point)

=item [2] B<mnt_type>

Mount type (aka file system type)

=item [3] B<mnt_opts>

Mount options, separated by colon.
...
I<setmntent(3)> or the equivalent of your platform upon initialization,
...
All methods raise exception I<FsQuota.error> upon errors. The exception
class is derived from B<OSError> and thus contains firstly a numerical
error code in attribute I<errno> (copied from I<errno> in most cases), and
secondly a derived error message in attribute I<strerror>.

Note the error string is adapted to the context of quota operations and
therefore not always identical to the text returned by B<strerror(3)>.
The normal error descriptions don't always make sense for quota errors
(e.g. B<ESRCH>: I<No such process>, here: I<No quota for this user>)
