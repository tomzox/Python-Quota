...
=head1 SYNOPSIS
    ...
    FsQuota::query(dev [,uid [,kind]])

    (block_curr, block_soft, block_hard, block_timelimit,
     inode_curr, inode_soft, inode_hard, inode_timelimit) =
    FsQuota::rpcquery(host, path [,uid [,kind]])

    FsQuota::rpcpeer([port [,use_tcp [,timeout]]])
    
    FsQuota::rpcauth([uid [,gid [,hostname]]])

    FsQuota::setqlim(dev, uid, block_soft, block_hard,
                   inode_soft, inode_hard [,tlo [,kind]])

    FsQuota::sync([dev])

    arg = FsQuota::getqcarg([path])

    FsQuota::setmntent()
    (dev, path, type, opts) = FsQuota::getmntent()
    FsQuota::endmntent()

=head1 DESCRIPTION

The B<FsQuota> module provides access to file system quotas.
...
=item I<(bc,bs,bh,bt, ic,is,ih,it) = FsQuota::query(dev, uid, kind)>
...
have to worry about that; use B<FsQuota::getqcarg> to automatically
...
of argument may also be produced by B<FsQuota::getqcarg>. Note: RPC
...
systems FsQuota::query will return undef in that case and
...
=item I<FsQuota::setqlim(dev, uid, bs,bh, is,ih, tlo, kind)>

Sets quota limits for the given user. Meanings of I<dev>, I<uid>,
I<bs>, I<bh>, I<is> and I<ih> are the same as in B<FsQuota::query>.
...
I<FsQuota::rpcquery(host,path,uid,kind)>

This is equivalent to B<FsQuota::query("host:path",uid,kind)>, i.e.
...
=item I<FsQuota::rpcpeer(port,use_tcp,timeout)>
...
=item I<FsQuota::rpcauth(uid,gid,hostname)>
...
=item I<arg = FsQuota::getqcarg(path)>

Get the required I<dev> argument for B<FsQuota::query> and B<FsQuota::setqlim>
...
a device file to B<FsQuota::query> (see also above at I<FsQuota::query>).

=item I<FsQuota::setmntent()>

Opens or resets the mount table. This is required before the first
invocation of B<FsQuota::getmntent>.
...
=item I<(dev, path, type, opts) = FsQuota::getmntent()>
...
accessible as a file at all, i.e. only via B<FsQuota::getmntent>).

=item I<FsQuota::endmntent()>

Close the mount table. Should be called after the last use of
B<FsQuota::getmntent> to free possibly allocated file handles and memory.
...
Quota query and limit functions raise I<FsQuota.error> upon errors.  The class
contains the numerical error code (copied from I<errno> in most cases) and an
error message (smilar B<strerror(3)>, however with some adaptions since the
normal messages don't always make sense for quota errors, e.g. I<ESRCH>: B<No
such process>, here: B<No quota for this user>)


B<FsQuota::endmntent> always returns I<None>.  All other functions
return 0 upon success, non-zero integer otherwise. As usual B<errno>
contains the error code, which should be translated using B<FsQuota::strerr>.

=head1 EXAMPLES

An example for each function can be found in the test script
I<test.pl>. See also the contrib directory for more examples.
